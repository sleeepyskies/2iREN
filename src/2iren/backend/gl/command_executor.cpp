...
GlCommandExecutor::GlCommandExecutor(const RenderResourceState& state)
    : m_state(state),
      m_tracked_state() { }
...
auto GlCommandExecutor::execute_buffer_upload(
    ...
    switch (desc.usage) {
        ...
        case BufferUsage::Stream: {
            ...
            std::memcpy(
                static_cast<u8*>(ptr) + cmd.dest_offset,
                data_slice.data(),
                data_slice.size()
            );
            break;
...
auto GlCommandExecutor::bind_vertex_buffer(
...
auto GlCommandExecutor::bind_index_buffer(
...
auto GlCommandExecutor::bind_uniform_buffer(
