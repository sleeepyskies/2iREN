...
auto Window::glfw_mouse_button_callback(GLFWwindow* window, const i32 button, const i32 action, const i32) -> void {
    const auto* self = static_cast<Window*>(glfwGetWindowUserPointer(window));
...
auto Window::glfw_scroll_callback(GLFWwindow* window, const f64 xoffset, const f64 yoffset) -> void {
    const auto* self = static_cast<Window*>(glfwGetWindowUserPointer(window));
