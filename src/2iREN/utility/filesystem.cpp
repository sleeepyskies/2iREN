...
File::File(const Path& path, const FileOpenMode mode) :
    m_mode(mode), m_path(path), m_size(std::nullopt) {
...
File::File(File&& other) noexcept :
    m_mode(other.m_mode), m_path(std::move(other.m_path)), m_size(other.m_size),
...
auto File::read_all_text() -> std::optional<std::string> {
    return read_all().transform([](const std::vector<u8>& buffer) {
...
auto to_virtual(const Path& path, const std::string_view v_key) -> std::optional<Path> {
    ...
    return get_physical_path(v_key).transform([&v_key, &path](const Path& physical_path) {
        std::error_code ec;
...
auto exists(const Path& path) -> bool {
    return to_physical(path)
        .transform([](const Path& p) {
            std::error_code ec;
...
auto get_file_size(const Path& path) -> std::optional<u64> {
    return to_physical(path).transform([](const Path& p) {
        std::error_code ec;
...
auto read_bytes(const Path& path) -> std::optional<std::vector<u8>> {
    return get_file_size(path).transform([&path](const u32 size) {
...
auto open(const Path& path, FileOpenMode mode) -> std::optional<File> {
    return to_physical(path).transform([mode](const auto& p) { return File{p, mode}; });
}
