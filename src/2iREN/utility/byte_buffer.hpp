...
/// TODO: not everything here can be constexpr, since we use reinterpret_cast<>()
/// as well as std::vector<>, but I'm too lazy to check what for now.
...
public:
    ...
    template <typename T>
    constexpr explicit ByteBuffer(const std::span<T>& data) {
    ...
    template <typename T>
    constexpr explicit ByteBuffer(const std::initializer_list<T> items) {
        write<T>(items);
    }
    ...
    [[nodiscard]]
    constexpr auto size_bytes() const noexcept -> usize {
        return m_data.size();
    }
    ...
    template <typename T>
    [[nodiscard]]
    constexpr auto size_as() const noexcept -> usize {
        return m_data.size() / sizeof(T);
    }

    /// @brief Returns buffer capacity in bytes.
    [[nodiscard]]
    constexpr auto capacity_bytes() const noexcept -> usize {
    ...
    template <typename T>
    [[nodiscard]]
    constexpr auto capacity_as() const noexcept -> usize {
        return m_data.capacity() / sizeof(T);
    }

    /// @brief Resizes the buffer in bytes.
    constexpr auto resize_bytes(const usize size_bytes) -> void {
        m_data.resize(size_bytes);
    }

    /// @brief Resizes the buffer for T.
    template <typename T>
    constexpr auto resize_as(const usize size_t) -> void {
        m_data.resize(size_t* sizeof(T));
    }

    /// @brief Reserves memory in bytes.
    constexpr auto reserve_bytes(const usize size) noexcept -> void {
        m_data.reserve(size);
    }

    /// @brief Reserves memory for N elements of type T.
    template <typename T>
    constexpr auto reserve_as(const usize size) noexcept -> void {
    ...
    template <typename T>
    constexpr auto write(const std::initializer_list<T> items) -> void {
    ...
    /// @brief Writes the items into the buffer.
    template <typename T>
    constexpr auto write(const std::span<T> items) -> void {
        for (const auto& item : items) {
            write(item);
        }
    }
    ...
    [[nodiscard]]
    constexpr auto view() const noexcept -> ByteBufferView {
...
private:
    ...
    template <typename T>
    constexpr auto assert_alignment() const -> void {
