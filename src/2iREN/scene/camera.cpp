...
auto Camera::projection_view() const noexcept -> Mat4x4f {
    const auto perspective = Mat4x4f::perspective(
        m_fov.get().to_radians(),
        m_aspect,
        m_nearplane,
        m_farplane
    );
    ...
    const auto position = m_position - Point3f{};
    const auto tx       = -Vec3f::dot(m_right, position);
    const auto ty       = -Vec3f::dot(m_up, position);
    const auto tz       = -Vec3f::dot(m_front, position);

    // clang-format off
    const auto view = Mat4x4f{{
        m_right.x, m_up.x, m_front.x, 0.f,
        m_right.y, m_up.y, m_front.y, 0.f,
        m_right.z, m_up.z, m_front.z, 0.f,
        tx,        ty,     tz,        1.f,
    }};
    // clang-format on
...
auto Camera::lookat(const Point3f at) noexcept -> void {
    ...
    m_yaw          = Radians{std::atan2(dir.z, dir.x)}.to_degrees();
...
auto Camera::update_vectors() noexcept -> void {
    ...
    m_right = Vec3f::normalize(Vec3f::cross(Vec3f::UP(), m_front));
    m_up    = Vec3f::normalize(Vec3f::cross(m_front, m_right));
}
