...
class LoadState {
public:
    [[nodiscard]] constexpr auto get_main() const noexcept -> LoadStatus {
        return m_main;
    }
    ...
    [[nodiscard]] constexpr auto get_deps() const noexcept -> LoadStatus {
...
class AssetServer {
    ...
    struct AssetServerData {
        ...
        /// in principle stateless. Furthermore, we assume all loaders are registered at engine
        /// init. If this is not the case, and loaders are added at runtime, there can be issues.
        Loaders loaders;
...
public:
    ...
    [[nodiscard]] auto get(const StrongHandle<A> handle) -> A* {
        ...
        if (!is_loaded_with_dependencies(handle)) {
            log::debug(
                "Could not get asset {}, as at least of of its dependencies is still loading.",
                handle
            );
            return nullptr;
        }
        ...
        return m_data.storage.run(
            [&handle](
                const std::unordered_map<TypeID, std::unique_ptr<AssetPoolBase>>& storage
            ) -> A* {
                const auto it = storage.find(AssetId::type_id<A>());
                if (it == storage.end()) {
                    log::debug(
                        "{} asset storage does not exist, cannot get asset.",
                        typename_of<A>(),
                        handle
                    );
                    return nullptr;
    ...
    /// @param path An optional path parameter. Useful if adding as asset that was loaded external
    /// to the server.
    ...
    [[nodiscard]] auto add(std::unique_ptr<A>&& asset, const AssetPath& path = AssetPath::invalid())
        ...
        return m_data.storage.run_exclusive([asset = std::move(asset),
                                             &path](auto& storage) mutable {
            const auto it = storage.find(AssetId::type_id<A>());
            if (it == storage.end()) {
                log::error(
                    "Could not find an appropriate asset pool for type {}",
                    typename_of<A>()
                );
        ...
        return m_data.default_handles.run(
            [](const std::unordered_map<TypeID, std::any>& default_handles) {
                ...
                if (it == default_handles.end()) {
                    log::error(
                        "There exists no default for asset type {}, but it was requested.",
                        typename_of<A>()
                    );
                    return StrongHandle<A>::invalid();
                }
    ...
    auto register_default(std::unique_ptr<A>&& asset) -> void {
        ...
        log::info(
            "Registering a new default for type {}. Default handle: {}",
            typename_of<A>(),
            handle
        );
...
private:
    ...
    auto on_asset_unload(const AssetId id) -> void {
        ...
        auto hashed_string = m_data.id_to_path.run_exclusive(
            [&](auto& map) -> std::optional<HashedString> {
        ...
        log::debug(
            "Asset from cache with path {} and id {} unloaded",
            hashed_string.value().data(),
            id
        );
    ...
    auto ensure_asset_registered() {
        ...
        auto contains = m_data.storage.run([tid](const auto& storage) -> bool {
        ...
        m_data.storage.run_exclusive([tid, this](auto& storage) {
            (void)storage
                .emplace(tid, std::make_unique<AssetPool<A>>([this](const AssetId id) {
                             on_asset_unload(id);
                         }))
                .first;
        });
...
public:
    ...
    template <IsAsset A>
    [[nodiscard]] auto add_labeled_asset(const std::string& label, std::unique_ptr<A>&& asset)
        -> StrongHandle<A> {
    ...
    template <IsAsset A>
    auto fetch_default() -> StrongHandle<A> {
    ...
    [[nodiscard]] constexpr auto device() noexcept -> Device& {
        return m_device;
    }
    /// @brief Returns the @ref WeakHandle this LoadContext was made to load an asset for.
    [[nodiscard]] constexpr auto handle() noexcept -> WeakHandle {
        return m_handle;
    }
...
private:
    auto notify_dependents(
        ...
        if (!asset_info.dependencies.empty()) {
            return;
        }
...
[[nodiscard]] auto AssetServer::load(
    ...
    if (!loader) {
        log::warn(
            "Could not load asset of type {}, as there exists no loader for it.",
            typename_of<A>()
        );
        return StrongHandle<A>::invalid();
    }
    ...
    m_data.id_to_path.run_exclusive([&](auto& map) {
        map[weak_handle.id()] = path.hashed_string();
    });

    // spawn new loading task
    ThreadPool::get().submit([this, path, loader, weak_handle, config = std::move(config)] mutable {
        const auto result = loader->load(
            LoadContext{*this, path, weak_handle, m_device},
            std::move(config)
        );
        if (!result) {
