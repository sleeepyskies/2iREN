...
/// https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html#indices-and-names For a brief overview
/// of GLTF see: https://www.khronos.org/files/gltf20-reference-guide.pdf
...
static auto gltf_index_type_to_siren(const cgltf_component_type type) -> IndexFormat {
    switch (type) {
        ...
        case cgltf_component_type_r_32f:
            PANIC("Attempted to define either a Int8, Int16 or Float32 as an index type.");
...
static auto check_gltf_primitive(const cgltf_primitive& primitve) -> AssetLoadError {
    if (primitve.type != cgltf_primitive_type_triangles) {
        log::warn(
            "2iREN only supports primitive type triangles. Encountered cgltf_primitive_type: {}",
            (u32)primitve.type
        );
        return std::unexpected(AssetErrorCode::NotSupported);
    }
...
static auto load_vertex_buffer(const cgltf_primitive& primitive, Device& device) -> VertexBuffer {
    ...
    // underlying gltf buffers. Eventually, shader permuations would be nice, but for now we do
    // this.
    ...
    for (usize i = 0; i < primitive.attributes_count; i++) {
        ASSERT(
            count == primitive.attributes[i].data->count,
            "gltf vertex attributes must all have the same count!"
        );
    }
    ...
    for (usize i = 0; i < primitive.attributes_count; i++) {
        ...
        switch (const auto type = attribute.type) {
            ...
            case cgltf_attribute_type_custom: {
                log::warn(
                    "gltf attribute contains unsupported cgltf_attribute_type: {}, skipping this.",
                    (usize)type
                );
                break;
            }
...
static auto load_nodes(
    ...
    const auto get_node_idx = [data](const cgltf_node* node_ptr) -> usize {
        return node_ptr - data->nodes;
    };
