...
AssetPath::AssetPath(
    const std::string_view vfs, const std::string_view relative_path, const std::string_view label
...
auto AssetPath::parse(const std::string& str) noexcept -> AssetPath {
    ...
    std::string path =
        str.substr(colon + 3, pound == std::string::npos ? std::string::npos : pound - (colon + 3));
...
auto AssetPath::relative_path() const -> std::string {
    return m_buffer->substr(
        m_path_offset, m_label_offset == 0 ? std::string::npos : m_label_offset - m_path_offset
    );
}
...
auto AssetPath::filename() const -> std::string {
    return Path{relative_path()}.filename().string();
}

auto AssetPath::extension() const -> std::string {
    return Path{relative_path()}.extension().string().substr(1);
}
