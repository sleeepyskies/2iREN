...
class Mutex {
public:
    using Guard = UniqueGuard<T>;
    using Lock  = Guard::LockType;
     ...
     */
    [[nodiscard]] auto try_lock() const noexcept -> std::expected<Guard, ResourceLocked> {
     ...
     */
    template <typename Function>
    auto run_guarded(Function&& func) const noexcept
        -> std::invoke_result_t<Function, UniqueGuard<T>&> {
...
public:
     ...
     * @return A @ref UniqueGuard on success, or @ref MutexError if locked.
     */
    [[nodiscard]] auto try_lock() const noexcept -> std::expected<Lock, ResourceLocked> {
        Lock lock{m_mutex, std::try_to_lock};
        if (!lock.owns_lock()) {
            return std::unexpected(ResourceLocked{});
        }
    ...
    auto run(Function&& func) noexcept -> std::invoke_result_t<Function> {
        auto guard = lock();
