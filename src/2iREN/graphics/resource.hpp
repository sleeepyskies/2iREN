...
struct ResourceHandle : Identifier<ResourceHandle<Tag>> {
     ...
     */
    ResourceHandle(const IndexType idx, const GenerationType gen) noexcept :
        Base(idx, gen, 0) { }
...
public:
    ...
    RenderResource(Device* device, HandleType handle) :
    ...
    auto to_string() const noexcept -> std::string {
        return std::format(
            "{}({})", typename_of<Resource>(), m_handle.packed()
        );
    }
...
template <
    typename ApiHandle,
    typename Resource,
    typename Details = detail::Nothing>
class RenderResourceTable {
public:
...
public:
    ...
    auto link(
        ...
        ASSERT(
            ...
            "passed an invalid ProxyHandleType: {}",
            proxy_handle
        );
    ...
    auto reserve_link(const ApiHandleType api_handle, Details&& details)
        -> HandleType {
    ...
    [[nodiscard]] auto fetch(const HandleType proxy_handle) const noexcept
        -> ApiHandleType {
        ASSERT(
            is_valid_id(proxy_handle), "Attempting to fetch an invalid handle."
        );
...
private:
    ...
    auto is_valid_id(const HandleType proxy_handle) const -> bool {
        if (proxy_handle.index()
            >= m_table.size()
            || !proxy_handle.is_valid()) {
            return false;
        }
...
struct std::hash<siren::ResourceHandle<Tag>> {
    auto operator()(const siren::ResourceHandle<Tag>& identifier) const noexcept
        -> siren::usize {
