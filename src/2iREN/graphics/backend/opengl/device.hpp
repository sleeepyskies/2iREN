...
/// @brief Encapsulates a mapped buffer pointer. This is used in streamed @ref
/// Buffer's.
...
struct GlSwapchainDetails {
    ...
    /// @brief An @ref RenderTarget. Since OpenGL abstracts the swapchain away,
    /// we just render to a mock this.
...
/// @brief Used to cache and retrieve FBOs for the OpenGL backend. Since FBOs
/// are vk/gl specific, they do not exist in sirens main API. Instead, we use
/// render targets, which consist of images. However, OpenGL requires rendering
/// to an FBO, so we use this to create/fetch FBOs based on images.
///
/// @todo @note Cached framebuffers are currently never cleaned up. do this
/// homie
class FramebufferCache {
public:
...
struct RenderResourceState {
    ...
    /// vertex array. This is because OpenGL has no notion of a Pipeline, but we
    /// use a VA in the pipeline.
