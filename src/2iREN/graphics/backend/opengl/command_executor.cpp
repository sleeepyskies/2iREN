...
OpenGLCommandExecutor::OpenGLCommandExecutor(const RenderResourceState& state) :
    m_state(state) { }
...
auto OpenGLCommandExecutor::execute(RenderPass&& pass) -> void {
    ...
    if (descriptor.target.colors.size()
        > 0
        || descriptor.target.depth_stencil
        != std::nullopt) {
        const GLuint framebuffer =
            m_state.framebuffer_cache.get_create_for(descriptor.target);
        ...
        for (const auto& [index, attachment] :
             std::views::enumerate(descriptor.target.colors)) {
            if (attachment.begin_operation == BeginOperation::Clear) {
                ...
                glInvalidateNamedFramebufferData(
                    framebuffer, 1, &attachment_enum
                );
            }
        }

        if (descriptor.target.depth_stencil.has_value()) {
            if (descriptor.target.depth_stencil->begin_operation
...
auto OpenGLCommandExecutor::bind_graphics_pipeline(
    const BindGraphicsPipeline& bind
) const -> void {
    ...
    const auto& pipeline_descriptor =
        pipeline_table.details(bind.pipeline_handle).descriptor;
...
auto OpenGLCommandExecutor::bind_vertex_buffer(
    ...
    const auto vbo =
        m_state.buffer_table.fetch(bind_vertex_buffer.vertex_buffer);
...
auto OpenGLCommandExecutor::bind_uniform_buffer(
    ...
    const auto ubo =
        m_state.buffer_table.fetch(bind_uniform_buffer.uniform_buffer);
...
auto OpenGLCommandExecutor::bind_uniform_buffer_range(
    ...
    m_statistics.count_bind_uniform_buffer++;
    const auto ubo =
        m_state.buffer_table.fetch(bind_uniform_buffer_range.uniform_buffer);
    glBindBufferRange(
        GL_UNIFORM_BUFFER,
...
auto OpenGLCommandExecutor::bind_shader_storage_buffer(
    ...
    const auto ubo = m_state.buffer_table.fetch(
        bind_shader_storage_buffer.shader_storage_buffer
    );
    glBindBufferBase(
        GL_SHADER_STORAGE_BUFFER, bind_shader_storage_buffer.slot, ubo
    );
}

auto OpenGLCommandExecutor::bind_sampled_image(
    const BindSampledImage& bind_sampled_image
) const -> void {
    m_statistics.count_bind_sampled_image++;
    const auto img = m_state.image_table.fetch(bind_sampled_image.image);
    const auto& desc =
        m_state.image_table.details(bind_sampled_image.image).descriptor;

    switch (desc.dimension) {
        case ImageDimension::D1:
            PANIC("bind_sampled_image doesnt support D1 yet");
        ...
        case ImageDimension::D3:
            PANIC("bind_sampled_image doesnt support D2 yet");
...
auto OpenGLCommandExecutor::bind_storage_image(
    const BindStorageImage& bind_storage_image
) const -> void {
    m_statistics.count_bind_storage_image++;
    const auto img = m_state.image_table.fetch(bind_storage_image.image);
    const auto& desc =
        m_state.image_table.details(bind_storage_image.image).descriptor;
    glBindImageTexture(
...
auto OpenGLCommandExecutor::begin_query(const BeginQuery& begin_query) const
    -> void {
    const auto kind =
        m_state.query_table.details(begin_query.query).descriptor.kind;
...
auto OpenGLCommandExecutor::end_query(const EndQuery& end_query) const -> void {
    const auto kind =
        m_state.query_table.details(end_query.query).descriptor.kind;
    const auto apikind = opengl::query_kind_to_gl(kind);
    glEndQuery(apikind);
}

auto OpenGLCommandExecutor::draw_arrays(const DrawArrays& draw_arrays) const
    -> void {
...
auto OpenGLCommandExecutor::draw_indexed(const DrawIndexed& draw_indexed) const
    -> void {
