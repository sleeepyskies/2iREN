...
auto MetalCommandExecutor::execute(RenderPass&& pass) -> void {
    ...
    for (usize i = 0; i < descriptor.target.colors.size(); i++) {
        ...
        mtl_attachment->setLoadAction(
            metal::load_action(attachment.begin_operation)
        );
        mtl_attachment->setClearColor(
            MTL::ClearColor::Make(rgba.r, rgba.g, rgba.b, rgba.a)
        );
    ...
    m_cmd_encoder = m_cmd_buffer->renderCommandEncoder(desc);
...
auto MetalCommandExecutor::bind_graphics_pipeline(
    ...
    auto* pipeline_state =
        m_state.pipelines.fetch(bind_graphics_pipeline.pipeline_handle);
...
auto MetalCommandExecutor::bind_vertex_buffer(
    ...
    m_cmd_encoder->setVertexBuffer(
        buf, bind_vertex_buffer.offset, bind_vertex_buffer.slot
    );
}

auto MetalCommandExecutor::draw_arrays(const DrawArrays& draw_arrays) -> void {
    m_cmd_encoder->drawPrimitives(
        metal::primitive_type(draw_arrays.primitive_topology),
        draw_arrays.start,
        draw_arrays.count
    );
}
