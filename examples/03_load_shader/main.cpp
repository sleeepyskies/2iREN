...
int main() {
    ...
    {
        const auto start = siren::time::elapsed_ms();
        siren::log::info("Waiting for shader to finish loading...");
        while (!server.is_loaded_with_dependencies(shader_handle)) { /* do nothing */ }
        const auto end = siren::time::elapsed_ms();
        siren::log::info("Shader successfully loaded, took {}ms.", end - start);
    }
